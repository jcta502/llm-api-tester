...
export function createHttpServer({ root, profileStore, historyStore, settingsStore, themeHandler, token = '', updateHandler, backupExport, backupImport, onSettingsChanged }) {
  ...
  const server = createServer(async (req, res) => {
    ...
    try {
      ...
      if (req.method === 'POST' && pathname === '/api/profiles/reveal') {
        if (!profileStore) return sendJson(res, 400, { error: '浏览器开发模式不提供配置库，请启动桌面应用。' })
        const raw = await readBody(req)
        const { id } = JSON.parse(raw || '{}')
        if (typeof id !== 'string' || !/^[0-9a-f-]{36}$/i.test(id)) return sendJson(res, 400, { error: '配置标识无效。' })
        return sendJson(res, 200, await profileStore.reveal(id))
      }

      if (req.method === 'POST' && pathname === '/api/profiles/compat') {
        if (!profileStore) return sendJson(res, 400, { error: '浏览器开发模式不提供配置库，请启动桌面应用。' })
        const raw = await readBody(req)
        const { id } = JSON.parse(raw || '{}')
        if (typeof id !== 'string' || !/^[0-9a-f-]{36}$/i.test(id)) return sendJson(res, 400, { error: '配置标识无效。' })
        return sendJson(res, 200, await checkService.checkCompatibility(id))
      }

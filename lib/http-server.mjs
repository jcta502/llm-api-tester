...
export function createHttpServer({ root, profileStore, themeHandler, token = '' }) {
  ...
  const server = createServer(async (req, res) => {
    ...
    try {
      ...
      if (req.method === 'POST' && pathname === '/api/profiles/probe') {
        ...
        if (!profile.apiKey) return sendJson(res, 200, { ok: false, status: 400, error: '该配置没有保存密钥。', diagnosis: { code: 'missing_key', message: '缺少已保存的密钥', hint: '编辑配置并重新输入 API Key。' } })
        const result = await probePayload({ ...profile, action, model })
        return sendJson(res, 200, rendererProbeResult(result, profile.apiKey))
      }

      if (req.method === 'POST' && pathname === '/api/profiles/run') {
        if (!profileStore) return sendJson(res, 400, { error: '浏览器开发模式不提供配置库，请启动桌面应用。' })
        const raw = await readBody(req)
        const { jobId, ids } = JSON.parse(raw || '{}')
        ...
        try {
          ...
          }, { ...batchJobOptions(broadcast, jobId), signal: controller.signal })

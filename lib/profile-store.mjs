...
export class ProfileStore {
  ...
  // reveal() intentionally returns the decrypted secret to the caller. This is
  // the one place where a saved key leaves safeStorage unmasked — it exists so
  // the user can re-read a key they previously stored. The route/IPC layer
  // remains the only caller; the public list() never includes it.
  async reveal(id) {

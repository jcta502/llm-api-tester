...
function scrub(value, secret = '', limit = TEXT_LIMIT) {
...
export function rendererProbeResult(result = {}, secret = '') {
  ...
  if ('schemaOk' in result) safe.schemaOk = Boolean(result.schemaOk)
  if (result.schemaIssue) safe.schemaIssue = scrub(result.schemaIssue, secret)
  if (result.raw !== undefined) {
    const rawText = typeof result.raw === 'string' ? result.raw : JSON.stringify(result.raw, null, 2)
    safe.raw = scrub(rawText, secret, RAW_LIMIT)
  }
  if (result.rawText !== undefined) safe.rawText = scrub(result.rawText, secret, 4000)
